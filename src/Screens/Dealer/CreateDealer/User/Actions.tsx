...
const Actions = ({ row, users, DealershipDetails }: any) => {
    const dispatch = useAppDispatch();
    const { breakPoints, user, updateDealerUser, dealers, updatedDealer, dealerUsers } = useAppSelector((state) => state);
            ...
            <Box display="flex" alignItems="center" gap={2}>
                {screen?.IsEdit ? (
                    <Tooltip title="Edit" arrow>
                        <Box display="flex" alignItems="center" gap={1} sx={{ width: 30, cursor: "pointer" }} onClick={handleEdit}>
                            <BiEdit fontSize={24} />
                    ...
                    >
                        <Box display="flex" gap={1}>
                            ...
                            <Button
                                ...
                                disabled={row?.RoleId === 3 && !selectedUser?.value}
                                variant="filled"
                                sx={{ display: "flex" }}
                            >
                                Yes
